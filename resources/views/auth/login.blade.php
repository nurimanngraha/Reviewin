...
@section('body')
    <div
        class="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
        <!-- Subtle Background Glows -->
        <div class="absolute -top-40 -right-40 w-96 h-96 bg-brand-600/30 rounded-full blur-3xl pointer-events-none"></div>
        <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-emerald-600/20 rounded-full blur-3xl pointer-events-none">
        </div>
        ...
        <div class="sm:mx-auto sm:w-full sm:max-w-md relative z-10 px-4">
            <div class="text-center mb-8">
                <a href="{{ route('home') }}" class="inline-block group hover:scale-105 transition-transform">
                    <img src="{{ asset('assets/CreTechlogin.svg') }}" alt="CreTech"
                        style="width: {{ $loginLogoWidth }}; max-width: 280px; height: auto;"
            ...
            <div class="bg-white/95 backdrop-blur-xl py-8 px-6 shadow-2xl rounded-2xl sm:px-10 border border-white/20">
                @if (!empty($redirect))
                    <div
                        class="mb-5 p-3 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-2.5 text-xs text-amber-900 font-medium">
                        <svg class="w-4 h-4 text-amber-600 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span>Setelah login berhasil, Anda akan langsung diarahkan untuk melanjutkan proses aktivasi
                            perangkat.</span>
                    </div>
                @endif
                ...
                <form action="{{ route('login.submit') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <label for="password"
                            class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Password</label>
                    ...
                    <button type="submit"
                        class="w-full py-3 px-4 rounded-xl font-semibold text-white bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-700 hover:to-indigo-700 shadow-lg shadow-brand-600/30 transition-all text-sm mt-2">
                        Masuk Sekarang
                    </button>
                </form>
                ...
                <div class="mt-5 text-center text-xs text-slate-600">
                    ...
                    <a href="{{ route('register', !empty($redirect) ? ['redirect' => $redirect] : []) }}"
                        class="font-semibold text-brand-600 hover:text-brand-700 underline">Daftar sekarang</a>
                </div>
            </div>
        </div>
    </div>
